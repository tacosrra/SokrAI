...
```bash
IS_SANDBOX=1 archon workflow run <workflow-name> --branch <branch-name> "<message>"
```

**CRITICAL RULES**:

0. **Always prefix with `IS_SANDBOX=1`** — The VPS runs as root. Claude Code blocks `--dangerously-skip-permissions` under root without this env var. Always include `IS_SANDBOX=1` before any `archon` command.
...
IS_SANDBOX=1 archon workflow run archon-fix-github-issue --branch fix/issue-10 "Fix issue #10"
IS_SANDBOX=1 archon workflow run archon-fix-github-issue --branch fix/issue-11 "Fix issue #11"
IS_SANDBOX=1 archon workflow run archon-fix-github-issue --branch fix/issue-12 "Fix issue #12"
```
